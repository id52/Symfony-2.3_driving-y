...
class PassFilialController extends AbstractEntityController
{
    protected $listFields = array('region', 'title');
    protected $orderBy = array('region' => 'ASC', 'title' => 'ASC');
    protected $tmplItem = 'PassFilial/item.html.twig';
    protected $tmplSettings = 'PassFilial/settings.html.twig';
    ...
    public function mapsAction(Request $request)
    {
        ...
        if (!isset($request->get('form')['region'])) {
            ...
            return $this->render('AppBundle:Admin:PassFilial/maps.html.twig', array(
        ...
        $file_name_map = 'pass_filial_map.'.$region_id.'.png';

        $form_factory = $this->container->get('form.factory');
        /** @var $fb \Symfony\Component\Form\FormBuilder */
        $fb = $form_factory->createNamedBuilder('pass_filial', 'form', null, array(
            'translation_domain' => 'pass_filial',
        ));
        ...
        $fb->add('submit', 'submit', array(
            'label'              => 'admin.buttons.save',
        ...
        if ($form->isValid()) {
            ...
            $this->redirect($this->generateUrl('admin_pass_filials_maps'));
        }
    ...
    public function itemAction(Request $request)
    {
        ...
        /** @var $entity \My\AppBundle\Entity\PassFilial */

        $phones = (array)$request->get('pass_filial_phones');
        if ($phones) {
            $entity->setPhones(array());
            $phones_active = $request->get('pass_filial_phones_active');
        ...
        $emails = (array)$request->get('pass_filial_emails');
        if ($emails) {
            $entity->setEmails(array());
            $emails_active = $request->get('pass_filial_emails_active');
        ...
        $groups = (array)$request->get('pass_filial_groups');
        if ($groups) {
            $new_groups = array();
            $groups_active = $request->get('pass_filial_groups_active');
            foreach ($groups as $key => $value) {
                ...
                if ($row1 && $row2) {
                    $new_groups[] = array(
                        'row1'   => $row1,
                        'row2'   => $row2,
                        'active' => isset($groups_active[$key]),
                    );
                }
            }
            $entity->setGroups($new_groups);
        }

        $coords = (array)$request->get('pass_filial_coords');
        if ($coords) {
        ...
        if ($request->isMethod('post')) {
            ...
            $image = $this->em->find('AppBundle:Image', $image_id);

            if ($form->isValid()) {
                $this->em->persist($entity);

                if ($entity->getImage()) {
                    $entity->getImage()->setPassFilial(null);
                }
                if ($image) {
                    $image->setPassFilial($entity);
                }
    ...
    {
        $fb->add('opacity_not_active_pass_filials', 'integer');
        $fb->add('pass_time_recreating', 'integer');
        return $fb;
    }
    ...
    {
        $data['opacity_not_active_pass_filials'] = max(0, $data['opacity_not_active_pass_filials']);
        $data['opacity_not_active_pass_filials'] = min(100, $data['opacity_not_active_pass_filials']);
    }
}
